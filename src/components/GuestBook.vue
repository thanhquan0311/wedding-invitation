...
<template>
  <section class="w-full bg-slate-100 pt-5">
    <section class="container-section bg-slate-100">
      <HeaderSection title="Đăng ký tham dự" subtitle="Để sự kiện diễn ra suôn sẻ, đề nghị các khách mời đảm bảo có mặt tại sự kiện của chúng tôi" />
      <!-- Form -->
      <form 
        ref="form"
        @submit="sendMessage"
        ...
        <!-- Guest Status -->
        <div class="input-wrapper" data-aos="zoom-in">
          <label for="GuestStatus">Sự hiện diện</label>
          <select v-model="GuestStatus" name="GuestStatus" id="GuestStatus" required>
            <option value="Hadir">Hiện diện</option>
